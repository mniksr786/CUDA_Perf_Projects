...
int runCpuConv (int argc, char ** argv) {
	...
	std::chrono::duration<double> time_span = (tEnd- tStart);
	std::cout << "CPU CONV takes " << time_span.count() << " seconds.";
...
int convLayer_cpu( float * input, TensorShape iShape, float * filter, TensorShape fShape,    
                ...
                for (uint32_t y = 0; y < oShape.width; ++y) {   
                        ...
                        for (uint32_t j = 0; j < fShape.width; ++j) {   
                            for (uint32_t k = 0; k < fShape.channels; ++k) {   
                                ...
                                if (in_x >= 0 && in_x < iShape.height && in_y >= 0 && in_y < iShape.width) {   
                                    ...
                                    input[n * iShape.channels * iShape.height * iShape.width +   
                                          k * iShape.height * iShape.width +   
                                          in_x * iShape.width + in_y] *   
                                    filter[m * fShape.channels * fShape.height * fShape.width +   
                                           k * fShape.height * fShape.width +   
                                           i * fShape.width + j];   
                    ...
                    if (args.activation) {   
                        output[n * oShape.channels * oShape.height * oShape.width + 
                               m * oShape.height * oShape.width + 
                               x * oShape.width + y] =   
                        Activation(output[n * oShape.channels * oShape.height * oShape.width + 
                                          m * oShape.height * oShape.width + 
                                          x * oShape.width + y]);   
...
int runCpuGemm (int argc, char ** argv) {
	...
	TensorShape aShape = {1, 1, batchSize, 4096};
	TensorShape bShape = {1, 1, 4096, 4096};
	TensorShape cShape;
	GemmLayerArgs args = {32, 32, 1};
	...
	std::chrono::duration<double> time_span = (tEnd- tStart);
	std::cout << "CPU GEMM " << time_span.count() << " seconds.";
...
int gemmLayer_cpu (float * a, TensorShape aShape, float * b, TensorShape bShape, float * c, TensorShape & cShape, GemmLayerArgs & args) {
