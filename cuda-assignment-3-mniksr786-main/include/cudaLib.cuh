...
#define CUDA_LIB_H
	...
	//	Uncomment this to disable error counting
		...
		#define CUDA_ERR_CHECK 0
		#define USE_UNIFIED_MEMORY 0
		#define VALIDATE_GEMM 0		
		#define TILE_SIZE 32
	 ...
	 * @return int 
	 */
	extern int convLayer_gpu_kernel ( float * input, TensorShape iShape, 
		float * filter, TensorShape fShape, 
		float * bias, float * output, TensorShape oShape, 
		ConvLayerArgs & args);
	
	extern __global__ void convKernel (float* input, TensorShape iShape, float* filter, 
	TensorShape fShape, float* bias, float* output, TensorShape oShape, ConvLayerArgs args);
	
	extern __device__ float gpu_Activation(float x);
	...
	extern __global__ void gemmKernel (float *a, float *b, float *c, 
					int M, int N, int K);
	...
	extern void gemmCpu(float* a, float* b, float* c, int M, int N, int K);
	extern bool compareMatricesChunk(float* A, float* B, int M, int N);
